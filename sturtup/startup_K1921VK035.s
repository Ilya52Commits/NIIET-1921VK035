...
Stack_Size      EQU     0x00000400 ; объявление константы Stack_Size равной 1024 байт 

                AREA    STACK, NOINIT, READWRITE, ALIGN=3 ; создание отдельной секции в память с названием STACK
...
Stack_Mem       SPACE   Stack_Size ; выделяет пространство заданного размера в область стека
                ; SPACE - резервирует заполненное нулями место в памяти

__initial_sp ; создание метки, которая будет служить начальным значением указателя стека 
...
Heap_Size       EQU     0x00000200 ; объявление константы Heap_Size оавглй 512 байт

                AREA    HEAP, NOINIT, READWRITE, ALIGN=3 ; создание отдельной секции в память с названием HEAP

__heap_base ; начало кучи

Heap_Mem        SPACE   Heap_Size

__heap_limit ; конец кучи

                PRESERVE8 ; указывает компоновщику сохранять 8-байтное выравнивание стека
...
; Vector Table Mapped to Address 0 at Reset
                ...
                ; инструкция DCD созраняет 32-битное слово в памяти 
...
__Vectors_Size  EQU  __Vectors_End - __Vectors
                ...
                ; CODE содержит фактический код

; Reset handler
Reset_Handler    PROC
                 EXPORT  Reset_Handler             [WEAK]
     IMPORT  __main
     IMPORT  SystemInit ; выполняет начальную инициализацию процессора
                 LDR     R0, =SystemInit 
                 BLX     R0
                 LDR     R0, =__main ; передаётся контроль функции main
                 BX      R0
                 ENDP
...
NMI_Handler     PROC
                ...
                ; WEAK говорит о том, что эта функция может быть переопределена где-нибудь в другом месте проекта
                ; Это позволяет определять свои собственные обработчики прерываний

                B       .
                ; выполняет переход на свой же адрес, тем самым генерируя бесконечный цикл

                ENDP ; обозначает конец функции
HardFault_Handler\
                PROC
